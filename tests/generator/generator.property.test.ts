...
describe("Property 1: generated credentials pass validate_credential", () => {
  it("every generated credential has zero severity:error entries from both validators", async () => {
    let successCount = 0;
    await fc.assert(
...
describe("Property 2: generator terminates for all maxDepth values", () => {
  it("generate() always resolves for any maxDepth in [0, 10]", async () => {
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 0, max: 10 }), async (maxDepth) => {
        const generator = new CredentialGraphGenerator();
...
describe("Property 3: serialization round-trip preserves the credential", () => {
  it("double JSON round-trip produces a deeply equal value to single round-trip", async () => {
    let successCount = 0;
    await fc.assert(
      fc.asyncProperty(validConfigArb, async (config) => {
        const generator = new CredentialGraphGenerator();
...
describe("Property 4: coverage percentages are always in [0, 100]", () => {
  it("every percentage in CoverageReport satisfies 0 <= p <= 100 for any valid config", async () => {
    let successCount = 0;
    await fc.assert(
      fc.asyncProperty(validConfigArb, async (config) => {
        const generator = new CredentialGraphGenerator();
...
describe("Property 6: cardinality correctness in full mode", () => {
  ...
  it("array-schema properties emit arrays of length 1–5, single-value properties are not arrays", async () => {
    let successCount = 0;
    await fc.assert(
      fc.asyncProperty(
        fc.record({
        ...
        async (config) => {
          ...
          expect(Array.isArray(doc["@context"]), "@context should be an array").toBe(true);

          // type is always an array (OB3 requires it)
          expect(Array.isArray(doc.type), "type should be an array").toBe(true);

          // id is always a string (not an array)
          expect(Array.isArray(doc.id), "id should NOT be an array").toBe(false);
...
describe("Property 7: Mermaid has one node per class, one edge per distinct triple", () => {
  ...
  const edgeArb = fc.record({
    from: fc.constantFrom("AchievementCredential", "AchievementSubject", "Achievement", "Profile"),
    to: fc.constantFrom("AchievementSubject", "Achievement", "Profile", "Evidence"),
    propertyName: fc.constantFrom("credentialSubject", "achievement", "creator", "parentOrg"),
  });
  ...
  it("emits exactly one node declaration per distinct class name", () => {
    fc.assert(
      fc.property(activePathArb, (activePath) => {
        ...
        const nodeDeclarationLines = lines.filter((line) => /^\s+\w+\[[^\]]+\]$/.test(line));
  ...
  it("emits exactly one arrow line per distinct (from, to, propertyName) triple", () => {
    fc.assert(
      fc.property(activePathArb, (activePath) => {
        const mermaid = renderer.render(activePath);
        const lines = mermaid.split("\n");
        ...
        const distinctTriples = new Map<
          string,
          { from: string; to: string; propertyName: string }
        >();
        ...
        const arrowLines = lines.filter((line) => /^\s+\w+ -->\|[^|]+\| \w+$/.test(line));
        ...
        for (const [, edge] of distinctTriples) {
          ...
          const matchingArrows = arrowLines.filter((line) => line.trim() === expectedArrow);
...
describe("Property 5: Seeded runs produce identical output", () => {
  ...
  it("running generate twice with the same seeded config produces identical credential documents", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          maxDepth: fc.integer({ min: 0, max: 10 }),
